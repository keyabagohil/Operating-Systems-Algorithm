...
void priority()
{
    ...
    fscanf(fp, "%d", &n);
    for (int i = 0; i < n; i++) {
        fscanf(fp, "%d %d %d %d",&processes[i].pid, &processes[i].arrival_time, &processes[i].burst_time,&processes[i].priority);
    ...
    while (completed != n) {
        ...
        for (int i = 0; i < n; i++) {
            if (processes[i].arrival_time <= time && processes[i].is_completed == 0) {
                if (smallest == -1 || processes[i].priority < processes[smallest].priority) {
                    smallest = i;
                }
            }
